	...
	?>
		...
		?>
			...
			<form method="post" action="">
				...
				<input name="cont1" id="cont1" type="text" /><br />
				<span class="kop4"><?php echo $lang['general']['description']; ?></span><br />
				<textarea name="cont2" id="cont2" cols="50" rows="5" /></textarea><br />
...
function albums_page_admin_modifyalbum() {
	global $lang, $var1, $cont1, $cont2;
	if(file_exists(ALBUMS_DIR.'/'.$var1.'.php')) {
		include (ALBUMS_DIR.'/'.$var1.'.php');
		if (!isset($album_desc))
			$album_desc = '';
			?>
			<label class="kop2" for="cont1"><?php echo $lang['albums']['edit_album'] ?></label>
			<form method="post" action="">
				<span class="kop4"><?php echo $lang['albums']['choose_name']; ?></span><br />
				<input name="cont1" id="cont1" type="text" value="<?php echo $album_name; ?>" /><br />
				<span class="kop4"><?php echo $lang['general']['description']; ?></span><br />
				<textarea name="cont2" id="cont2" cols="50" rows="5" /><?php echo htmlspecialchars_decode($album_desc); ?></textarea><br />
				<input type="submit" name="submit" value="<?php echo $lang['general']['save']; ?>" />
			</form>
	<p>
		<a href="?module=albums">&lt;&lt;&lt; <?php echo $lang['general']['back']; ?></a>
	...
	//When form is submitted.
		if (isset($_POST['submit'])) {
			if (!empty($cont1) && seo_url($cont1) != $var1 && file_exists(ALBUMS_DIR.'/'.seo_url($cont1)))
				show_error($lang['albums']['name_exist'], 1);
			...
			elseif (!empty($cont1) && seo_url($cont1) != $var1) {
				...
				//Create and chmod directories.
				rename(ALBUMS_DIR.'/'.$var1, ALBUMS_DIR.'/'.$cont1);
				chmod(ALBUMS_DIR.'/'.$cont1, 0777);
				rename(ALBUMS_DIR.'/'.$var1.'.php', ALBUMS_DIR.'/'.$cont1.'.php');
				chmod(ALBUMS_DIR.'/'.$cont1.'.php', 0777);
			...
			elseif (!empty($cont1) && seo_url($cont1) == $var1) {
				...
				//Create album file.
				save_file(ALBUMS_DIR.'/'.$var1.'.php', $data);
				redirect('?module=albums', 0);
			}
		}
	}
